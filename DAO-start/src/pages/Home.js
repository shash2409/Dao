...
const Home = () => {
  ...
  async function createProposal(newProposal) {
    let options = {
      contractAddress: "0xbcf166722317e466431499A198f2e72ce6443Ba3",
    ...
    if (isInitialized) {
      ...
      const fetchTokenIdOwners = async () => {
        const options = {
          ...
          token_id:
            "896596920413805403331356585510736197301047076234078977105080513274820165647",
          chain: "mumbai",
        };
        const tokenIdOwners = await Web3Api.token.getTokenIdOwners(options);
        const addresses = tokenIdOwners.result.map((e) => e.owner_of);
        setVoters([0x01FB74b41e6DCc1E7234b44220B35EEC0b993686]);
      };
